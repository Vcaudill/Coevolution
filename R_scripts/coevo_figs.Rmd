...
Experiment 1: varies both mutation rates and mutation effect sizes.
Experiment 2: mutation rate was fixed for both species in each simulation, but allows the species to have different mutation effect sizes (and hence mutational variance).
Experiment 3: mutational variance was the same for newts and snakes in each simulation, although polygenicity could be different (by varying mutation rate and mutation effect size). 

## How to use this file?
 
 - run all of the simulations
 - run gather_data.Rmd to make lit (phenotypes, population size, ...), cor (correlations), and grid (values calculated from grids) file
 - all files will be read in through the Coevolution folder
 - All text files used to generate this markdown are included in the Coeovlution folder.

## Functions 

 - find_coevo_slope = calculating the slope of coevolution
...
values = c("Newt_mean_Pheno" = "darkred","Snake_mean_Pheno" = "steelblue"),
labels = c("Newt","Snake"))+
...
setwd("~/Desktop/Coevolution")
no_interaction_lit<-  read.csv("./Experiment1/flat_map/no_interaction/csv/all_lit.txt" , header = TRUE)
...
no_hair_lit<-  read.csv("./Experiment1/flat_map/no_hair/csv/all_lit.txt" , header = TRUE)
...
GA1_lit<-  read.csv("./Experiment1/flat_map/normal/csv/all_lit.txt" , header = TRUE)
...
no_interaction_lit_s=find_coevo_slope(file, coevo_begin_start_gen, coevo_begin_end_gen, coevo_endlength_gen, close_percent)
file=no_hair_lit
no_hair_lit_s=find_coevo_slope(file, coevo_begin_start_gen, coevo_begin_end_gen, coevo_endlength_gen, close_percent)
file=GA1_lit
GA1_lit_s=find_coevo_slope(file, coevo_begin_start_gen, coevo_begin_end_gen, coevo_endlength_gen, close_percent)

```

### Speeds of Coevolution 

```{r, look at the change in phenotyp the plots V4}
file= GA1_lit
choices=c("K", "F", "E")
min_gen=100
max_gen=50000
...
subt="" 
x_label= "Generations"
y_label= "Phenotype (Blue: Snake Resistance, Red: Newt Toxicity)"
p1=fast_slow_no_coevolution(file, choices, min_gen, max_gen, title, subt, x_label, y_label)
ggsave("figures/three_speeds_of_coevolution.pdf", plot=p1, height=10, width=10, units=c("in"), dpi=600)
plot(p1)
```
...
```{r, look at the change in phenotyp the plots V2}

title = "Phenotype Change, FLat Map"
y_label= "Change in Newt Toxicity"
x_label = "Newt GA"
coevo_boxplo_newt=coevo_slope_flat_map_newt_change(GA1_lit_s, title, subt, x_label, y_label)
ggsave("figures/coevo_boxplo_newt.pdf", plot=coevo_boxplo_newt, height=4, width=6, units=c("in"), dpi=600)
plot(coevo_boxplo_newt)
```

### Change in Snake Phenotype by Different Genetic Architectures 

```{r, look at the change in phenotyp the plots V3}
title = "Phenotype Change, FLat Map"
y_label= "Change in Snake Resistance"
x_label = "Snake GA"
coevo_boxplo_snake=coevo_slope_flat_map_snake_change(GA1_lit_s, title, subt, x_label, y_label)
ggsave("figures/coevo_boxplo_snake.pdf", plot=coevo_boxplo_snake, height=4, width=6, units=c("in"), dpi=600)
plot(coevo_boxplo_snake)
```
...
y_label = "Mean Change in Toxicity and Resistance"

three_experiments = boxplot_coevo_slope_small_exp_color(no_interaction_lit_s, no_hair_lit_s, GA1_lit_s, title, x_label, y_label)
ggsave("figures/three_experiments.pdf", plot=three_experiments, height=7, width=10, units=c("in"), dpi=600)
plot(three_experiments)
```

## {-}

## Spatial Phenotype Correlation for Experiment 1 {.tabset}

What is the correlation between newt toxicity and snake resistance across geographical space within the simulation?
Simulation varies both mutation rates and mutation effect sizes (Experiment 1). In these simulations there were different maps that created heterogeneous landscapes (cost gradient, interaction rate gradient).
...
GA1_cor<- read.csv("./Experiment1/cost_grade/both/csv/all_cor.txt" , header = TRUE)
...
cost_grade_cor<-  read.csv("./Experiment1/cost_grade/both/csv/all_cor.txt" , header = TRUE)
...
interrate_grade<-  read.csv("./Experiment1/interaction_grade/csv/all_cor.txt" , header = TRUE)
...
cost_grade_no_interact_cor<-  read.csv("./Experiment1/cost_grade/both_no_interaction/csv/all_cor.txt" , header = TRUE)
...
snake_change_cor<-  read.csv("./Experiment1/cost_grade/snake_change/csv/all_cor.txt" , header = TRUE)
...
newt_change_cor<-  read.csv("./Experiment1/cost_grade/newt_change/csv/all_cor.txt" , header = TRUE)
...
```{R, plotting the correlation boxplots created above}

boxplot_cors_by_gen_col_rep = boxplot_cors_by_gen_col_rep(file1, file2, file3, title, x_label, y_label, min_gen, max_gen)

ggsave("figures/three_correlation.pdf", plot=boxplot_cors_by_gen_col_rep, height=8, width=11, units=c("in"), dpi=600)
plot(boxplot_cors_by_gen_col_rep)
```

### Spatial phenotype correlation NoInteractionCostMap/SnakeCostMap/NewtCostMap

```{R, plotting the correlation boxplots created above part2}
...
ggsave("figures/three_correlation_app.pdf", plot=boxplot_cors_by_gen_col_rep_other3, height=8, width=11, units=c("in"), dpi=600)
plot(boxplot_cors_by_gen_col_rep_other3)
```

### Spatial phenotype correlation overtime FlatMap/CostMap/InteractionMap

```{R, plotting the correlation boxplots created above part3}
...
ggsave("figures/cor_over_time.pdf", plot=cors_overtime_col_rep, height=8, width=11, units=c("in"), dpi=600)
plot(cors_overtime_col_rep)
```

## {-}

## Reading in the grid files for the cost gradient {.tabset}

What do local newt and snake phenotypes look like within the simulation?
A few simulations from Experiment 1 (varies both mutation rates and mutation effect sizes) with a cost heterogeneous landscapes. 

```{r, read in grid files 2}
require(readr)
setwd("~/Desktop/Coevolution")
cost_grade_grid<- read.csv("./Experiment1/cost_grade/both/csv/all_grid.txt" , header = TRUE) %>%  filter(between(generation, 40000, 50000))
...
ggsave("figures/grid_example.pdf", plot=grid_example, height=7, width=5, units=c("in"), dpi=600)
plot(grid_example)
```

### Min Phenotypes for the Cost Gradent: Snake 4a, Newt 3a

```{r, min grid Snake 4a, Newt 3a}
file=cost_grade_grid
file_name="1e-11_5_1e-10_0.5_0"
spegen=44501
type="min"
...
ggsave("figures/grid_min_example.pdf", plot=grid_min_example, height=7, width=5, units=c("in"), dpi=600)
plot(grid_min_example)
```

### Min Phenotypes for the Cost Gradent: Snake 2a, Newt 4a

```{r, min grid Snake 2a, Newt 4a}
file=cost_grade_grid
...
ggsave("figures/grid_min_example2-4.pdf", plot=grid_min_example2, height=7, width=5, units=c("in"), dpi=600)
```

### Min Phenotypes for the Cost Gradent: Snake 4a, Newt 2a

```{r, min grid Snake 4a, Newt 2a}
file=cost_grade_grid
...
plot(grid_min_example42)
ggsave("figures/grid_min_example4-2.pdf", plot=grid_min_example42, height=7, width=5, units=c("in"), dpi=600)

```

### Mean Phenotypes for the Cost Gradent: Snake 2a, Newt 4a

```{r, mean grid Snake 2a, Newt 4a}
...
ggsave("figures/grid_mean_example2-4.pdf", plot=grid_mean_example2, height=7, width=5, units=c("in"), dpi=600)
```

### Mean Phenotypes for the Cost Gradent: Snake 4a, Newt 2a

```{r, mean grid Snake 4a, Newt 2a}
...
grid_mean_example2 = main_grid_plot(file_name, spegen, file, selected_title=title, type, row_num, col_num, mymaxlimit)
plot(grid_mean_example2)
ggsave("figures/grid_mean_example4-2.pdf", plot=grid_mean_example2, height=7, width=5, units=c("in"), dpi=600)
...
although polygenicity could be different (by varying mutation rate and mutation effect size). 
...
ggsave("figures/coevo_speed.pdf", plot=coevo_speed, height=7, width=10, units=c("in"), dpi=600)
plot(coevo_speed)

```

## Relationship between population size and phenotype {.tabset}
...
```{r, looking at the realtionship between pop size and phenotype early E1}
...
ggsave("figures/Early_Popsize_vs_pheno.pdf", plot=wg1, height=10, width=10, units=c("in"), dpi=600)
wg1
...
ggsave("figures/Late_Popsize_vs_pheno.pdf", plot=wg2, height=10, width=10, units=c("in"), dpi=600)
wg2
```

### PopSize and Phenotype Experiment 2b Simulation

```{r, looking at the realtionship between pop size and phenotype early E2a}
...
```{r, looking at the realtionship between pop size and phenotype early E2b}
...
```{r, looking at the realtionship between pop size and phenotype early E2d}
...
```{r, looking at the realtionship between pop size and phenotype early E2e}
...
```{r, looking at the realtionship between pop size and phenotype early E3f}
...
```{r, looking at the realtionship between pop size and phenotype early E3g}
my_cur_file = v_b_lit
min_gen=5000
max_gen=10000
letter="g"
...
```{r, looking at the realtionship between pop size and phenotype early E3h}
my_cur_file = v_c_lit
min_gen=5000
max_gen=10000
letter="h"
...
```{r, looking at the realtionship between pop size and phenotype early E3i}
my_cur_file = v_d_lit
min_gen=5000
max_gen=10000
letter="i"
...
## Plotting the relationship between phenotype/population size and GA {.tabset}
...
ggplot(data_long) +
  ...
  scale_fill_manual(values=c(Newt_mean_Pheno="red",Snake_mean_Pheno="blue",dif="purple"),breaks=c('Newt_mean_Pheno','Snake_mean_Pheno','dif'), labels=c( "Newt Toxicity", "Snake Resistance", "Phenotype Difference (S-N)"), name="Mean")+
    theme(plot.title = element_text(size=5)) +
  scale_alpha(guide = 'none')+
  #labs(fill = " ")+
...
ggsave("figures/PD_PSD.pdf", plot=pg1, height=7, width=6, units=c("in"), dpi=600)
plot(pg1)
```
...
mas1 = plot_grid(mu_a, mu_b, mu_c, mu_d, labels = c('A', 'B', "C", "D"), label_size = 12)
ggsave("figures/mus_app.pdf", plot=mas1, height=7, width=10, units=c("in"), dpi=600)
plot(mas1)
```
...
ggsave("figures/vms_app.pdf", plot=pg2, height=7, width=10, units=c("in"), dpi=600)
plot(pg2)
